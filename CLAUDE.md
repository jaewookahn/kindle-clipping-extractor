...
⚠️ **쓰기도 기기에 도달하지 않는다** (2026-09-19 MTP 실측, 문서 §16.8).
마운트로 쓴 파일은 File Provider 로컬 캐시에만 남고 기기에 플러시되지 않는다.
**기기 파일 변경은 MTP 직접 쓰기(Delete + Send)만 신뢰 가능.** MacDroid 를
다시 켜면 캐시된 옛 내용이 기기를 덮을 수 있으므로, 재기동 후엔 MTP 재검증.
(같은 프로세스에서 MTP 세션을 두 번 열면 `libusb_claim_interface() = -3` —
`close()` 가 의도적으로 릴리스하지 않는다. 검증은 별도 프로세스로.)
...
## 에이전트 통신 규약 (토큰 절약 모드) — 필수

여러 세션이 함께 일할 때 적용합니다. 정본: `~/prj/reading_manager/COMMS.md`

1. **금지** — 코드, 수정 내역(diff), 에러 로그를 대화창에 직접 출력하지 않습니다.
2. **저장** — 작업 결과와 긴 설명은 반드시 로컬 파일로 남깁니다 (소스코드 또는 `*.md`).
3. **통신** — 보고·지시는 **[파일 경로] + [1~2줄 요약]**만 보냅니다.
   예) `KSDK 파서 조사 완료. 결과는 kindle/KSDK_SURVEY.md 참조.`

에이전트 간 메시지, 사용자 대면 보고, 커밋 메시지 모두 해당합니다. 근거·수치·표는 파일에 두고 메시지에는 **어디를 보라**만 적습니다.

예외: 한 줄 사실 확인, 즉시 판단이 필요한 경고, 선택지만 나열하는 결정 요청.

**커밋 주체 구분** — 여러 세션이 같은 저장소에 커밋하면 author 가 동일해 git 로그로 구분되지 않습니다. 커밋 메시지 trailer 에 세션명을 남깁니다: `Session: <세션명>`

**언어** — 사용자 대면 텍스트는 한국어 존댓말로 통일합니다 (답변·보고·질문·커밋 메시지·문서·에이전트 간 메시지).
